			...
			id = "<?php echo $module->id; ?>",
		 	validator = $('#contact-form_<?php echo $module->id; ?>').validate({
				...
				rules: {
					...
					message: {
						minlength: <?php echo $params->get('msg_minlength'); ?>
					}<?php if( $params->get('captcha_req')==1 ) { ?>,
		...
		<!-- Name Field -->
		 ...
		 <!-- Message Field -->
		  <div class="control-group control-group-textarea <?php echo $params->get('errors_position');?>">
			...
			<?php } ?>
			<div class="controls">
			  <textarea name="message" class="mod_tm_ajax_contact_form_textarea" id="inputMessage_<?php echo $module->id; ?>"<?php if($labels_pos) { ?> placeholder="<?php echo $params->get('message_name'); ?>"<?php } ?> required></textarea>
			</div>
		  </div>
		  <?php
